...
Document.prototype.init = function (doc, opts, fn) {
  ...
  // здесь надо понять как передать вместо _id
  // названия поля, которое является ключевым.
  if (doc._id && opts && opts.populated && opts.populated.length) {
    for (var i = 0; i < opts.populated.length; ++i) {
      var item = opts.populated[i];
      var id = undefined;
      // get exact key field as we really need
      if(item.options && item.options.refPk)
        id = doc[item.options.refPk[item.path]];
      else 
        id = String(doc._id);
...
Document.prototype.toObject = function(_options) {
  var options = clone(_options);
  if (options && options.depopulate && this.$__.wasPopulated) {
    var tkf = options.refPk;
    // populated paths that we set to a document
    if (tkf) {
      return clone(this[tkf], options);
    } else {
