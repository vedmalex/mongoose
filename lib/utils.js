...
function cloneObject (obj, options) {
  ...
  if (retainKeyOrder) {
    ...
    while (i--) {
      ...
      if(root){
        var propSchema = schema.path(k);
        if (propSchema) {
          if (propSchema.caster && propSchema.caster.options && propSchema.caster.options.refPk) {
            options.refPk = propSchema.caster.options.refPk;
          } else if (propSchema.options && propSchema.options.refPk) {
            options.refPk = propSchema.options.refPk;
          }
        }
      }
...
function RelateOptions(options) {
    if ( !(options.hasOwnProperty('relName')
        && options.hasOwnProperty('propName')
        && options.hasOwnProperty('from')
        && options.hasOwnProperty('to')
        && options.hasOwnProperty('ref')
        && options.hasOwnProperty('single')
        && options.hasOwnProperty('embedded')
        && options.hasOwnProperty('oppositeEmbedded')))
    throw new Error("Invalid Options for relate Operation");
    exports.merge(this, options);
}
// compatibility with utils.clone
RelateOptions.prototype.constructor = Object;

exports.RelateOptions = RelateOptions;
