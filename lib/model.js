...
function populate(model, docs, options, cb) {
  ...
  rawIds = getIdsForAndAddIdsInMapPopulate(model, modelsMap);
  ...
  promise = new Promise(function(err, vals, options, assignmentOpts) {
    ...
    var refPk = assignmentOpts.refPk, lean = options.options && options.options.lean,
      len = vals.length,
  ...
  for (i = 0; i < len; i++) {
    ...
    match[mod.options.refPk] || (match[mod.options.refPk] = {
      $in: ids
    });
    ...
    assignmentOpts.refPk = mod.options.refPk;
    ...
    if (assignmentOpts.excludeId) {
      ...
      if ('string' == typeof select) {
        ...
        delete select[mod.options.refPk];
      }
    }
...
function getIdsForAndAddIdsInMapPopulate(model, modelsMap) {
  ...
  for (j = 0; j < len2; j++) {
    ...
    for (i = 0; i < len; i++) {
      ret = undefined;
      doc = docs[i];
      id = String(utils.getValue(options.sourcePk, doc));
      isDocument = !! doc.$__;
      ...
      if (ret) {
        ret = convertTo_id(ret, options.sourcePk);
...
function assignVals(o) {
  ...
  var docs = o.docs;
  var path = o.path;
  var actualPath = o.actualPath;
  var rawIds = o.rawIds;
  var options = o.options;

  for (var i = 0, docsLen = docs.length; i < docsLen; ++i) {
    if (utils.getValue(actualPath, docs[i]) == null)
      continue;
