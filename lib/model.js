...
/**
 * return ModelName not [object Object]
 */
...
Model.prototype.save = function save (fn) {
  ...
  if (this.isNew) {
    ...
    // drop virtual rels to avoid it from saving in collection
    var vRels = Object.keys(this.schema.virtualRels);
    var i, rec;
    var len = vRels.length;
    for (i = 0; i< len; i++){
      delete obj[vRels[i]];
    }
...
function populate (model, docs, options, cb) {
  ...
  if (!schema && options.options && options.options.schema) {
    schema = model._getSchema(options.options.schema[path]);
    if(schema) actualPath = options.options.schema[path];
  } // there can be no schema at all if it is populated from association class and renamed using propName
  // but this is toobad for embedded association... schema must be to have caster... with ref strings
  if(!schema && options.options && !options.options.lean) {
    debugger;
    // catchs wrong definition for queries.
    console.log("not registered schema for fields" +path+':'+actualPath);
    throw new Error("not registered schema for fields" +path+':'+actualPath); 
  }
  var subpath;

  // empty Value default
  var emptyVal = schema && schema.options && Array.isArray(schema.options.type) ? [] : null;
  ...
  // get refPk
  var refPk = (schema && schema.options && schema.options.refPk) || '_id';
  var sourcePk = (schema && schema.options && schema.options.sourcePk) || '_id';
  ...
  for (i = 0; i < len; i++) {
    ret = undefined;
    doc = docs[i];
    id = String(utils.getValue(sourcePk, doc));
    isDocument = !! doc.$__;
    ...
    if (!ret || Array.isArray(ret) && 0 === ret.length) {
      ret = utils.getValue(actualPath, doc);
      if (!ret) {
        // if actual path is not actual store of values.
        ret = utils.getValue(path, doc);
      }
    }

    if (ret) {
      ret = convertTo_id(ret, sourcePk);
  ...
  Model.find(match, select, options.options, function (err, vals) {
    ...
    for (var i = 0; i < len; i++) {
      val = vals[i];
      key = String(utils.getValue(refPk, val));
      rawDocs[key] = val;
      rawOrder[key] = i;
...
function convertTo_id (val, sourcePk) {
  if (val instanceof Model) return val[sourcePk];
...
function maybeRemoveId (subdoc, assignmentOpts) {
  if (assignmentOpts.excludeId) {
    if ('function' == typeof subdoc.setValue) {
      subdoc.setValue(assignmentOpts.refPk, undefined)
    } else {
      delete subdoc[assignmentOpts.refPk];
    }
  }
}
