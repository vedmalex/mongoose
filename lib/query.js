...
var utils = require('./utils')
  ...
  , MongooseError = require('./error.js')
...
Query.prototype.execFind = function(callback) {
    ...
    model.collection.find(castQuery, options, function(err, cursor) {
        ...
        cursor.toArray(tick(getFindCallback(completeMany, self, options, model, fields, promise)));
    });
    return this;
};
...
function completeMany(model, docs, fields, self, pop, promise) {
    ...
    var lcb = function(err) {
        if (err) return promise.error(err);
        if (--count === 0) {
            QCB(self.options, arr, function(err, result) {
                if (err) return promise.error(err);
...
Query.prototype.findOne = function(callback) {
    ...
    model.collection.findOne(castQuery, options, tick(getFindCallback(completeOne, self, options, model, fields, promise)));
...
function completeOne(model, doc, fields, self, pop, promise) {
    ...
    casted.init(doc, opts, function(err) {
        if (err) return promise.error(err);
        QCB(self.options, casted, function(err, result) {
            if (err) return promise.error(err);
            promise.complete(result);
        });
    });
...
function fillObject(query, model, docs, options, callback) {
    ...
    if (!options.derived) {
        var keepUserOrder = query.options && query.options.sort || undefined;
        ...
        if (MissingSchemas[relationModel]) {
            // да удаляем
            delete query.options.populate[options.to];
        } else {
            // нет
...
function getFindCallback(completeAs, self, options, model, fields, promise) {
    return function(err, docs) {
        ...
        if (!self.options.populate && !self.options.relate) {
            // to len Query Language options live.
            return true === options.lean ? QCB(self.options, docs, function(err, result) {
                if (err) return promise.error(err);
                promise.complete(result);
        ...
        function done(err, docs) {
            if (err) return promise.error(err);
            // debugger
            var pop = self.options.populate ? helpers.preparePopulationOptions(self, options): null;
            return true === options.lean ? QCB(self.options, docs, function(err, result) {
                if (err) return promise.error(err);
                promise.complete(result);
...
// find({}).relate( {relName:"EmployeeGroups",propName:"Groups",from:"emp",to:"group",ref:"Group", single: true} ).populate(group)
Query.prototype.relate = function(options) {
    ...
    if (options && options.relName && options.propName && options.from && options.to && options.ref && options.hasOwnProperty('single')) {
        if (!this.options.relate) this.options.relate = {};
        this.options.relate[options.relName + options.propName + '(' + options.to + ')'] = new utils.RelateOptions(options);
        return this;
    } else {
