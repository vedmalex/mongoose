...
Query.prototype.find = function (conditions, callback) {
  ...
  if (mquery.canMerge(conditions)) {
    _this.merge(conditions);
  }

  prepareDiscriminatorCriteria(_this);

  try {
    _this.cast(_this.model);
    _this._castError = null;
  } catch (err) {
    _this._castError = err;
  }

  // if we don't have a callback, then just return the query object
  if (!callback) {
    return Query.base.find.call(_this);
  }
  ...
  if (_this._castError) {
    promise.error(_this._castError);
    return _this;
  }

  _this._applyPaths();
  _this._fields = _this._castFields(_this._fields);

  var fields = _this._fieldsForExec();
  var options = _this._mongooseOptions;

  var cb = getFindCallback(completeMany, _this, options, _this.model, fields, promise);

  if (_this.schema) {
    _this.model.hooks.execPre('find', _this, function(error) {
      if (error) {
        return cb(error);
      }
      Query.base.find.call(_this, {}, cb);
    });
    return _this;
  }

  return Query.base.find.call(_this, {}, cb);
}
...
function completeMany(model, docs, fields, self, pop, promise) {
    ...
    var lcb = function(err) {
        if (err) return promise.error(err);
        if (--count === 0) {
            QCB(self._mongooseOptions, arr, function(err, result) {
                if (err) return promise.error(err);
...
Query.prototype.findOne = function (conditions, projection, options, callback) {
  ...
  self.model.hooks.execPre('findOne', this, function(error) {
    if (error) {
      return promise.error(error);
    }
    // don't pass in the conditions because we already merged them in
    Query.base.findOne.call(self, {}, getFindCallback(completeOne, self, options, self.model, projection, promise))
  });
...
function completeOne(model, doc, fields, self, pop, promise) {
    ...
    casted.init(doc, opts, function(err) {
        if (err) return promise.error(err);
        QCB(self._mongooseOptions, casted, function(err, result) {
            if (err) return promise.error(err);
            promise.complete(result);
        });
    });
...
Query.prototype.relate = function(options) {
    // debugger
    var opts = this._mongooseOptions;
    if (options && options.relName && options.propName && options.from && options.to && options.ref && options.hasOwnProperty('single')) {
        if (!opts.relate) opts.relate = {};
        // opts.relate[options.relName + options.propName + '(' + options.to + ')'] = new utils.RelateOptions(options);
        opts.relate[options.propName] = new utils.RelateOptions(options);
        return this;
    } else {
        for (var relName in options) {
            this.relate(options[relName]);
        }
    }
};

function RelationError(original, path, detail) {
  var MongooseError = require('./error.js');
    MongooseError(this, 'Relation' + path + 'error :' + detail);
...
var dedupeArray = function(accum, current) {
...
var ModelCache = {};
var MissingSchemas = {};
function fillObject(query, model, docs, options, callback) {
    // debugger
    // no need to discover any thing all key already embedded here
    if (options.oppositeEmbedded && !options.derived) return callback(); //
    if (!options.derived) {
        var keepUserOrder = query._mongooseOptions && query._mongooseOptions.sort || undefined;
        // if not single result then it is a set.
        var resultSet = !options.single;

        var i, doc, id,
        ids = [];
        // it is used for both type queries find and findOne so
        if (docs && !Array.isArray(docs)) docs = [docs];
        var len = docs.length;
        var docsOrder = {};
        var fromKey = options.fromKeyField;
        for (i = 0; i < len; i++) {
            id = docs[i][fromKey] || utils.getValue(fromKey, docs[i]);
            ids.push(id);
            // save order
            docsOrder[id] = i;
        }

        var relationModel = options.relationModel;
        var relation;
        // проверяем не помещена ли модель по реляции в список отсутствующих моделей.
        if (MissingSchemas[relationModel]) {
            // да удаляем
            delete query._mongooseOptions.populate[options.to];
        } else {
            // нет
            relation = ModelCache[relationModel];
            if (!relation) {
                try {
                    relation = model.db.model(relationModel);
                    ModelCache[relationModel] = true;
                } catch (e) {
                    // если модель не найдена заносим ее в списко отсутствующих моделей
                    console.log("model not found ", relationModel, e);
                    // this relation isn't exists in code generation so we will not use it
                    MissingSchemas[relationModel] = true;
                    delete query._mongooseOptions.populate[options.to];
                }
            } else relation = model.db.model(relationModel);
        }
        // если реляция ссылается на не зарегистрированный объект ... выходим
        if ((relation === null) || (relation === undefined)) return callback();

        var selectionFields = !(options.embedded) ? options.to : options.toKeyField;
        var promiseFirst = new Promise();

        function getResult(err, data) {
            // debugger
            var result = {};
            if (err) return callback(new RelationError(err, options.relName, "relation query failed"));
            var i, opposit, from, val, optFrom = options.from;
            if (keepUserOrder) {
            ...
            if (resultSet) {
                for (i = 0; i < len; i++) {
                    opposit = data[i];
                    from = opposit[optFrom] || utils.getValue(options.from, opposit);
                    val = opposit[selectionFields] || utils.getValue(selectionFields, opposit);
                    if (!result[from]) result[from] = [];
                    if(!Array.isArray(from)){
                      result[from].push(val);
                    } else {
                      for (var j = 0, lenfrom = from.length; j < lenfrom; j++) {
                        result[from[i]].push(val);
                      }
                    }
                }
            } else {
            ...
            var key, keyList, origKeys, pathSchema;
            var keys = Object.keys(result);
            var optPName = options.propName;

            len = keys.length;
            for (i = 0; i < len; i++) {
                key = keys[i];
                if((docsOrder[key] !== undefined) && (docsOrder[key] !== null)) {
                  var resVal = result[key];
                  if(Array.isArray(resVal)){
                      resVal = resVal.reduce(dedupeArray, []);
                  }
                  utils.setValue(options.propName, resVal, docs[docsOrder[key]]);
                }
            }
            callback(err);
        }

        promiseFirst.addBack(getResult);
        ...
        relation.find(_query, selectionFields + " " + options.from, {
            lean: true//!options.embedded// подумать почему все так...
        }, promiseFirst.resolve.bind(promiseFirst));
    } else {
        if (!options.derivation || (options.derivation && options.derivation.mode == 'client')) callback();
        else {
            if (options.derivation && (options.derivation.mode == 'server') && options.derivation.Name) {
                console.log('+++++++++++++++++++++++++++++');
                try {
                    var funcName = options.derivation.Name.replace(/ /g, '_');
                    var deriver = model.prototype[options.derivation.Name];
                    var j, derdoc, cnt = 0;
                    var db = model.db;
                    ...
                    var setDerivedValue = function(doc) {
                        return function(err, data) {
                            utils.setValue(options.propName, data, doc);
                            next(err);
                        };
                    };
                    for (j = 0; j < docsLen; j++) {
                        derdoc = docs[j];
                        deriver(db, derdoc, setDerivedValue(derdoc));
                    }
                } catch (e) {
                    callback(e);
                }
            } else {
                callback("unknown type of derived association");
            }
        }
    }
}

function run(query, model, docs, relate, fn) {
    ...
    var cnt = 0;
    var keys = Object.keys(relate);
    var stlen = keys.length;
    ...
    for (var i = 0; i < stlen; i++) {
        relOp = relate[keys[i]];
        fillObject(query, model, docs, relOp, next);
    }
    if (stlen === 0) done();
}

function QCB(options, result, callback) {
  // debugger
    var many = Array.isArray(result);
    var ids = {}, resSet, inSet, i, item, lookup, itemUseGet, resLen, lookupUse; // 0 - alreadyId, 1 - get, 2 - direct access
    var relOpt = options.relate;
    // debugger
    if (relOpt && Object.keys(relOpt).length > 0) {
        for (var rk in relOpt) {
            var rel = relOpt[rk];
            var relKeyField = rel.toKeyField;
            // if (typeof(rel.populate) === 'object' && rel.populate.conditions && Object.keys(rel.populate.conditions).length > 0) {
            resSet = undefined;
            inSet = undefined;
            i = undefined;
            item = undefined;
            ...
            if (rel.ensure) {
                inSet = many ? result : [result];
                resSet = [];
                resLen = inSet.length;
                for (i = 0; i < resLen; i++) {
                    item = inSet[i];
                    lookup = utils.getValue(rel.to, item);
                    if (!rel.single ? (lookup && lookup.length > 0) : (lookup !== '' && lookup !== undefined && lookup !== null)) resSet.push(item);
                }
                result = many ? resSet : resSet[0];
            }
            ...
            if (rel.onlyIds) {
                // debugger
                ////нужно значительно упростить метод!!!!
                resSet = many ? result : [result];
                resLen = resSet.length;
                var newVal;
                for (i = 0; i < resLen; i++) {
                    item = resSet[i];
                    if (item) {
                        ...
                        if (lookup) {
                            if (rel.single) {
                                if (lookupUse === undefined) {
                                    if (lookup.hasOwnProperty(relKeyField)) lookupUse = 1;
                                    if (typeof(lookup.get) === 'function') lookupUse = 2;
                                    if (lookupUse === undefined) {
                                        lookupUse = 0;
                                        break;
                                    }
                                }
                                if (lookupUse === 2) newVal = lookup.get(relKeyField);
                                else newVal = lookup[relKeyField];

                            } else {
                                newVal = [];
                                var lookupresLen = lookup.length;
                                for (j = 0; j < lookupresLen; j++) {
                                    if (lookupUse === undefined) {
                                        if (lookup[j].hasOwnProperty(relKeyField)) lookupUse = 1;
                                        if (typeof(lookup[j].get) === 'function') lookupUse = 2;
                                        if (lookupUse === undefined) {
                                            lookupUse = 0;
                                            break;
                                        }
                                    ...
                                    if (lookupUse === 2) newVal.push(lookup[j].get(relKeyField));
                                    else newVal.push(lookup[j][relKeyField]);
                                }
                                if (lookupUse === 0) break;
                                if (newVal.length === 0) newVal = null;
                            }
                            if (newVal !== null && newVal !== undefined && newVal !== '') {
                                utils.setValue(rel.to, newVal, item);
                            }
                        }
                    }
                }
...
function getFindCallback(completeAs, self, options, model, fields, promise) {
    return function(err, docs) {
        // debugger
        if (err) return promise.error(err);

        if (!docs) return promise.complete(null);
        if (0 === docs.length) return promise.complete(docs);

        if (!self._mongooseOptions.populate && !self._mongooseOptions.relate) {
            // to len Query Language options live.
            return true === options.lean ? QCB(self._mongooseOptions, docs, function(err, result) {
                if (err) return promise.error(err);
                promise.complete(result);
            }) : completeAs(model, docs, fields, self, null, promise);
        }
        // подумать в какой последовательности надо это делать.
        var toRun = [];
        if (self._mongooseOptions.populate && !self._mongooseOptions.relate) {
            toRun.push(function(docs) {
                var pop = helpers.preparePopulationOptionsMQ(self, options);
                model.populate(docs, pop, next);
            });
        }
        if (self._mongooseOptions.relate) {
            var relate = helpers.prepareRelateOptions(self, options);
            toRun.push(function(docs) {
                run(self, model, docs, relate, next);
            });

            toRun.push(function(docs) {
                if (self._mongooseOptions.populate) {
                    var pop = helpers.preparePopulationOptionsMQ(self, options);
                    // debugger
                    model.populate(docs, pop, next);
                } else next(null, docs);
            ...
            for (var key in relate) {
                var rel = relate[key];
                var populate = rel.populate;
                if (populate) {
                    if ("object" === typeof(populate)) self.populate({path:rel.propName, select:populate.fields, model:rel.ref, match:populate.conditions, options:populate.options});
                    else self.populate({path:rel.propName, model:rel.ref});
                }
            }
            // debugger;
        }

        var cnt = toRun.length;

        function done(err, docs) {
            if (err) return promise.error(err);
            // debugger
            var pop = self._mongooseOptions.populate ? helpers.preparePopulationOptionsMQ(self, options): null;
            return true === options.lean ? QCB(self._mongooseOptions, docs, function(err, result) {
                if (err) return promise.error(err);
                promise.complete(result);
            }) : completeAs(model, docs, fields, self, pop, promise);
        }

        function next(err, docs) {
            if (err) return done(err, docs);
            if (cnt-- > 0)
              toRun.shift()(docs);
            else
              done(err, docs);
