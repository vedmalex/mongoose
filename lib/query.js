...
function fillObject(query, model, docs, options, callback) {
    ...
    if (!options.derived) {
        ...
        for (i = 0; i < len; i++) {
            id = docs[i][fromKey] || getValue(fromKey, docs[i]);
            ids.push(id);
            // save order
            docsOrder[id] = i;
        }
        ...
        function getResult(err, data) {
            ...
            if (resultSet) {
                for (i = 0; i < len; i++) {
                    opposit = data[i];
                    from = opposit[optFrom] || getValue(options.from, opposit);
                    val = opposit[selectionFields] || getValue(selectionFields, opposit);
            ...
            for (i = 0; i < len; i++) {
                key = keys[i];
                setValue(options.propName, result[key], docs[docsOrder[key]]);
            }
            callback(err);
        }
                ...
                try {
                    ...
                    var setDerivedValue = function(doc) {
                        return function(err, data) {
                            setValue(options.propName, data, doc);
                            next(err);
                        };
                    };
                    for (j = 0; j < docsLen; j++) {
...
Query.prototype.relate = function(options) {
    ...
    if (options && options.relName && options.propName && options.from && options.to && options.ref && options.hasOwnProperty('single')) {
        ...
        this.options.relate[options.relName + options.propName + '(' + options.to + ')'] = new RelateOptions(options);
        return this;
    } else {
